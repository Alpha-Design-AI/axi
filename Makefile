...
VCS_TARGETS := $(addsuffix .vcs.log,$(addprefix sim-,$(TBS)))


.SHELL: bash

.PHONY: help all sim_all sim_all_vcs clean
...
help:
	...
	@echo "compile_vcs.log:  compile files using VCS"
	@echo "sim-#TB#.log:     simulates a given testbench using Questasim, available TBs are:"
	@echo "$(addprefix ###############-#,$(TBS))" | sed -e 's/ /\n/g' | sed -e 's/#/ /g'
	@echo "sim-#TB#.vcs.log: simulates a given testbench using VCS, available TBs are:"
	@echo "$(addprefix ###############-#,$(TBS))" | sed -e 's/ /\n/g' | sed -e 's/#/ /g'
	@echo "sim_all:          simulates all available testbenches using Questasim"
	@echo "sim_all_vcs:      simulates all available testbenches using VCS"
	@echo ""
...
compile_vcs.log: Bender.yml | build
	export VCS="$(VCS)"; cd build && ../scripts/compile_vcs.sh | tee ../$@
	(! grep -n "Error:" $@)
